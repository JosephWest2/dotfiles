...
check_workspace_configs() {
    ...
    vocalinux_environment=$(<"$repo_dir/dot_config/environment.d/vocalinux.conf")
    ...
    assert_contains "$hyprland" 'hl.exec_cmd("google-chrome-stable", { workspace = "name:c silent" })' "hyprland chrome workspace"
    ...
    assert_contains "$hyprland" 'hl.workspace_rule({ workspace = "name:c", monitor = monitors.left, default = true })' "hyprland default browser workspace"
    ...
    assert_contains "$vocalinux_environment" 'VOCALINUX_FORCE_BACKEND=wtype' "vocalinux wtype backend"
...
check_profile() {
    ...
    case "$profile" in
        wet-leg)
            assert_ignored "$ignored" ".config/environment.d/vocalinux.conf" "$profile"
        ...
        MacBookAir)
            ...
            ;;
        joeyarchlinux)
            assert_included "$ignored" ".config/environment.d/vocalinux.conf" "$profile"
